...
export function nullable<T extends z.ZodType<unknown, string>>(schema: T) {
  return z
    ...
    .pipe(schema.nullable()) as z.ZodType<z.infer<T> | null, string>;
}

/** Non-empty text field (min 1 character) */
export const formText = z.string().min(1, 'text cannot be empty');
...
export function formPin(length = 6) {
  return z
    .string()
    .regex(new RegExp(`^\\d{${length}}$`), `pin must be ${length} digits`);
}
